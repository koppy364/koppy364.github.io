...
## 7.2 그렇다면 좋은 `hash function`는 어떠한 조건들을 갖추고 있어야 하는가?
...
공개 주소 방식이라고도 불리는 이 알고리즘은 Collision 이 발생하면 데이터를 저장할 장소를 찾아 헤맨다. Worst Case 의 경우 비어있는 버킷을 찾지 못하고 탐색을 시작한 위치까지 되돌아 올 수 있다. 이 과정에서도 여러 방법들이 존재하는데, 다음 세 가지에 대해 알아보자.
    ...
    3.  Double hashing probing
        하나의 해쉬 함수에서 충돌이 발생하면 2 차 해쉬 함수를 이용해 새로운  
        주소를 할당한다. 위 두 가지 방법에 비해 많은 연산량을 요구하게 된다.
    ```
...
### 7.3.1 연결 리스트를 사용하는 방식(Linked List)
...
### 7.3.2 Tree 를 사용하는 방식 (Red-Black Tree)
...
### 7.3.3 `Open Address` vs `Separate Chaining`    
...
## 8.1 정점과 간선의 집합, Graph

_cf) 트리 또한 그래프이며, 그 중 사이클이 허용되지 않는 그래프를 말한다._

## 8.2 그래프 관련 용어 정리

### 8.2.1 Undirected Graph 와 Directed Graph (Digraph)
...
### 8.2.3 가중치 그래프(Weight Graph)와 부분 그래프(Sub Graph)
...
### 8.4.1 깊이 우선 탐색 (Depth First Search: DFS)
...
### 8.4.2 너비 우선 탐색 (Breadth First Search: BFS)
